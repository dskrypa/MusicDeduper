...
class TagHeader:
   ...
   def setVersion(self, v):
      ...
      if isinstance(v, tuple) or isinstance(v, list):
         self.version = eyeD3b.utils.versionsToConstant(v);
         (self.majorVersion,
          self.minorVersion,
          self.revVersion) = v;
      # Handle int constants.
         ...
         (self.majorVersion,
          self.minorVersion,
          self.revVersion) = eyeD3b.utils.constantToVersions(v);
         self.version = v;
      else:
...
class Tag:
   ...
   def link(self, f, v = ID3_ANY_VERSION):
      ...
      fileName = "";
      if isinstance(f, file) or isinstance(f, SpooledTemporaryFile):
         fileName = f.name;
         self.linkedFile = LinkedFile(fileName, f);
      ...
      tagFound = 0;
      padding = 0;
      TRACE_MSG("Linking File: " + str(fileName));
      if v == ID3_V1:
   ...
   def remove(self, version = ID3_CURRENT_VERSION):
      ...
      tagFile = self.linkedFile.getFile();
      closeAfter = (tagFile == None);
      if closeAfter:
      ...
      if ((version & ID3_V2) or (version == ID3_ANY_VERSION)) and self.header.tagSize:
         ...
         if tagFile.read(3) == "ID3":
            ...
            tmpFile.write(tagFile.read());
            tagFile.seek(0);
            tagFile.truncate();
            tmpFile.seek(0);
            tagFile.write(tmpFile.read());
            tmpFile.close();            
            if closeAfter:
               tagFile.close();
   ...
   def getArtist(self, artistID = ARTIST_FIDS):
      frameIDs = artistID if isinstance(artistID, list) else [artistID];
      for fid in frameIDs:
         f = self.frames[fid];
         if f:
             return f[0].text;
   ...
   def strToUnicode(self, s):
       ...
       if t != unicode and t == str:
           s = unicode(s, eyeD3b.LOCAL_ENCODING);
...
class Genre:
   ...
   def set(self, id, name):
      ...
      if not utils.strictID3():
         ...
         try:
            if genres[name] != id:
               raise GenreException("eyeD3b.genres[" + str(id) + "] " +\
                                    "does not match " + name);
            self.id = id;
            self.name = name;
         except:
...
class TagFile:
   ...
   def rename(self, name, fsencoding):
       ...
       if os.path.exists(new_name):
           raise TagException("File '%s' exists, eyeD3b will not overwrite it" %
                              new_name)
   ...
   def getPlayTimeString(self):
      from eyeD3b.utils import format_track_time
...
class Mp3AudioFile(TagFile):
   ...
   def getBitRate(self):
      ...
      if xHead and xHead.vbr:
         tpf = eyeD3b.mp3.computeTimePerFrame(self.header);
...
def isMp3File(fileName):
    type = eyeD3b.utils.guess_mime_type(fileName);
   ...
   def __init__(self, fileName, f=None):
       if isinstance(fileName, str):
           try:
               ...
               # Work around the local encoding not matching that of a mounted filesystem
               self.name = fileName
       else:
           self.name = fileName;
       self.file = f;
